...
export default function EditItemScreen() {
  ...
  const [pieceCount, setPieceCount] = useState(item.piece_count?.toString() ?? "");
  const [playerCount, setPlayerCount] = useState(item.player_count?.toString() ?? "");
    ...
    <View className="flex-1 bg-surface-secondary dark:bg-surface-dark-secondary">
      ...
      >
        ...
        <View className="bg-surface dark:bg-surface-dark rounded-2xl border border-border dark:border-border-dark px-4 py-3 mb-6">
          <TextInput
            className="text-content dark:text-content-dark text-base"
            placeholder={type === "puslespill" ? "f.eks. Ravensburger" : "f.eks. Stonemaier Games"}
            placeholderTextColor="#A8A29E"
            value={brand}
                ...
                >
                  <Text
                    className={`text-sm font-medium ${
                      difficulty === opt ? "text-white" : "text-content dark:text-content-dark"
                    }`}
                  >
                    {opt}
                  </Text>
      ...
      >
        ...
        >
          {saving ? (
            ...
            <Text className="text-white text-base font-semibold">Lagre endringer</Text>
          )}
        </TouchableOpacity>
      </View>
    </View>
