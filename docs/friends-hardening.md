...
landing) + **MERGED to `main`** (2026-07-08); `feat/friends-hardening` branch
deleted (local + remote). DB (`regenerate_invite_code` + combined `accept_invite`)
...
1. **🔴 `App.tsx` — app could brick to a blank screen.** The F3 deep-link work
   ...
   removing the gate entirely — `AppContent` now renders `AuthScreen` _outside_
